...
A dynamic file-passing and auto navigation bash script for vifm (a vi-like file
manager), Vim, and a shell. It uses tmux for servers. It is invisible in the
shell; no navigation commands are shown. It is fairly extensible through
rebindings in vifm and vim. envi exists because of the efficiency of working
with the three major abstractions of data separately; text, commands, and
files. I didn't want ugly slow addons in Vim for navigation, or have to
close Vim to open multiple files or navigate. I often found myself opening all
three of the programs envi uses together, because vifm opens files in Vim
...
"terminal" variable in envi. The methods use to keep commands invisible require
a short PS1. To change this, set the PS1 variable to a short string like '\$'
when $TERM == screen\*. Other shell configuration can also be done this way.
...
ZZ or manually sending envi -q causes all the programs to exit.
If you "lose" a server, need to reload
